 ...
 * Change Logs:
 * Date           Author       Notes
 * 2022-05-15     lgnq         the first version
 */
...
static struct lpc17xx_spi_config spi_config[] =
{
#ifdef BSP_USING_SPI0
    SPI0_BUS_CONFIG,
#endif
...
static struct lpc17xx_spi spi_bus_obj[sizeof(spi_config) / sizeof(spi_config[0])] = {0};

static rt_err_t lpc17xx_spi_configure(struct lpc17xx_spi *spi_drv, struct rt_spi_configuration *cfg)
{
...
static rt_uint32_t spixfer(struct rt_spi_device *device, struct rt_spi_message *message)
{
    ...
    struct lpc17xx_spi *spi_drv =  rt_container_of(device->bus, struct lpc17xx_spi, spi_bus);
    struct lpc17xx_hw_spi_cs *cs = device->parent.user_data;
                              ...
                              struct rt_spi_configuration *configuration)
{
    ...
    return lpc17xx_spi_configure(spi_drv, configuration);
}

static const struct rt_spi_ops lpc17xx_spi_ops =
{
...
static int rt_hw_spi_bus_init(void)
{
    ...
    for (int i = 0; i < sizeof(spi_config) / sizeof(spi_config[0]); i++)
    {
        ...
        result = rt_spi_bus_register(&spi_bus_obj[i].spi_bus, spi_config[i].bus_name, &lpc17xx_spi_ops);
...
rt_err_t rt_hw_spi_device_attach(const char *bus_name, const char *device_name, uint32_t portindex, uint32_t cs_gpiobase, uint32_t cs_gpio_pin)
{
    ...
    struct lpc17xx_hw_spi_cs *cs_pin;
    ...
    cs_pin = (struct lpc17xx_hw_spi_cs *)rt_malloc(sizeof(struct lpc17xx_hw_spi_cs));
